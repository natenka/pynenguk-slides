...
* ```(([0-9a-fA-F]{4}\.){2}[0-9a-fA-F]{4})``` - сюди потрапить MAC-адреса
  * ```[0-9a-fA-F]{4}\.``` - ця частина описує 4 літери або цифри та точку
  * ```([0-9a-fA-F]{4}\.){2}``` - тут дужки потрібні, щоб вказати, що 4 літери або цифри і точка повторюються два рази
...
```python
In [4]: ​​match = re.search('((?:[0-9a-fA-F]{4}\.){2}[0-9a-fA-F]{4}).+vlan (\d+).+порт (\S+).+порт (\S+)', журнал)
```
...
При роботі з групами можна використати результат, який потрапив до групи, далі у тому ж таки виразі.

Наприклад, у висновку sh ip bgp останній стовпець описує атрибут AS Path (через які автономні системи пройшов маршрут):
```
bgp = '''
R9# sh ip bgp | be Мережа
   Метрика наступного переходу мережі LocPrf Weight Path
...
Припустимо, треба отримати ті префікси, у яких у дорозі кілька разів повторюється один і той самий номер AS.
...
У разі це група 1, вона єдина.

Крім того, у цьому виразі перед рядком регулярного виразу стоїть літера r.
Це так званий raw рядок.

---
### Повторення захопленого результату

Тут зручніше використовувати її, тому що інакше треба буде екранувати зворотний сліш, щоб посилання на групу спрацювало коректно:
```python
match = re.search('(\d+) \\1', line)
```

При використанні регулярних виразів краще завжди використовувати рядки рядка.
...
```python
In [3]: для рядка в bgp.split('\n'):
   ...: match = re.search(r'(\d+) \1 \1', лінія)
   ...: якщо збіг:
   ...: друк (line)
   ...:
* 192.168.66.0/24 192.168.79.7 0 500 500 500 i
...
Аналогічним чином можна посилатися на результат, який потрапив до іменованої групи:
```python
In [129]: для рядка в bgp.split('\n'):
...
У функції search два обов'язкові параметри:
* Regex - регулярне вираження
* string - рядок, в якому шукається збіг

Якщо збіг знайдено, функція поверне спеціальний об'єкт Match.
Якщо збігу не було, функція поверне None.

---
### Пошук функцій

При цьому особливість функції search у тому, що вона шукає лише перший збіг.
Тобто, якщо в рядку є кілька підрядків, які відповідають регулярному виразу, search поверне лише перший знайдений збіг.

---
### Пошук функцій

Найпростіший приклад регулярного вираження - підрядок:
```python
В [1]: імпорт ре

In [2]: int_line = 'MTU 1500 байт, BW 10000 Кбіт, DLY 1000 мкс,'

In [3]: match = re.search('MTU', int_line)
```

В даному випадку ми просто шукаємо, чи є підрядок 'MTU' у рядку int_line.

Якщо вона є, у змінній match буде знаходитись спеціальний об'єкт Match:
```python
In [4]: ​​print(match)
...
Об'єкт Match має кілька методів, які дозволяють отримувати різну інформацію про отриманий збіг.
Наприклад, метод group показує, що у рядку збіглося з описаним виразом.

В даному випадку це просто підрядок 'MTU':
```python
In [5]: match.group()
Out[5]: 'MAN'
```
...
Якщо збігу не було, у змінній match буде значення None:
```python
In [6]: int_line = 'MTU 1500 байт, BW 10000 Кбіт, DLY 1000 мкс,'
...
Повністю можливості регулярних виразів проявляються під час використання спеціальних символів.
Наприклад, символ ``\d`` означає цифру, а ``+`` означає повторення попереднього символу один або більше разів.
Якщо їх поєднати ``\d+```, вийде вираз, який означає одну або більше цифр.


---
### Пошук функцій

Використовуючи цей вираз, можна отримати частину рядка, в якому описано пропускну здатність:
```python
In [9]: int_line = 'MTU 1500 байт, BW 10000 Кбіт, DLY 1000 мкс,'
...
Особливо корисні Регулярні вирази в отриманні певних підрядків із рядка.
Наприклад, необхідно отримати VLAN, MAC та порти з виведення такого лог-повідомлення:
```python
In [12]: log2 = '3 жовтня 12:49:15.941: %SW_MATM-4-MACFLAP_NOTIF: Хост f04d.a206.7fd6 у vlan 1 перемикається між портом Gi0/5 і Gi0/16'
```

Це можна зробити за допомогою такого регулярного виразу:
```python
In [13]: re.search('Хост (\S+) у vlan (\d+) перемикається між портом (\S+) і портом (\S+)', log2).groups()
...
Метод групи повертає тільки ті частини вихідного рядка, які потрапили в круглі дужки.
Таким чином, уклавши частину виразу у дужки, можна вказати, які частини рядка треба запам'ятати.

Вираз ``\d+``` вже використовувався раніше - воно описує одну або більше цифр.
А вираз ``\S+``` описує всі символи, крім whitespace (пробіл, таб та інші).
