...
* робоча директорія - копія окремої версії проекту  определённой версии проекта
* індекс (staging area) - информация о том, что должно попасть в следующий commit
* директорія Git (.git) - тут хранятся метаданные и база данных объектов проект
